...
const {
    ...
    CLIENT_TIMEOUT = process.env.CLIENT_TIMEOUT || 5000,
    TCP_HEALTH_CHECK_TIMEOUT = process.env.TCP_HEALTH_CHECK_TIMEOUT || 3000,
    TCP_PROBE_PORT = process.env.TCP_PROBE_PORT || 22,
    ROUTE_TABLE_ID: string[] = process.env.ROUTE_TABLE_ID.split(',');
...
exports.main = async (context, req, res) => {
    ...
    if (!getPrimaryHealth && !getSecondaryHealth) {
        ...
        // If PIN_TO_INSTNACE enabled and both instances are healthy switch any routes to that instance.
        if (PIN_TO) {
...
export class RouteFailover {
    ...
    public async removeRoute(nextHopID, routeTableId, destinationCidrBlock): Promise<void> {
        ...
        try {
            ...
            throw console.error('Error in removeRoute. Error deleting route ', JSON.stringify(err));
        }
    }
